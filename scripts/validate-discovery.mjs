...
export async function validateDiscovery(taskDir, root, { phase = 'final', workbook } = {}) {
  ...
  if (manifest.项目目录 !== 'liveshow-proto' || manifest.生成策略 !== 'source-first' || manifest.目标范围?.模块名称 !== '直播模块') problems.push('发现任务身份或范围不正确');
  ...
  const source = await collectLiveEvidence(root);
  ...
  const replayed = discoverScenarios(replayedRules, pages), byId = new Map(replayedRules.规则.map(r => [r.规则标识, r]));
  replayed.场景.forEach(s => {designScenario(s, pages, byId);sourceContract(s,pages);});
  ...
  if (baseline.历史输入.length || baseline.规则SHA256 !== hash(rules) || baseline.场景SHA256 !== hash(library)) problems.push('原始发现阶段未与历史比较隔离');
  if (phase === 'final') {
    ...
    if (hash(coverage.记录) !== hash(mapCoverage(library.场景, candidate.测试用例))) problems.push('场景覆盖结论不能由实际用例重现');
