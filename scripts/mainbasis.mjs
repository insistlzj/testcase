...
const governance = ['AGENTS.md', 'Cem Kaner.txt', '全局证据扫描指令.md', '原型与需求清单同步指令.md', '流程追溯文件说明.md', 'scripts/mainbasis.mjs', 'scripts/requirement-traceability.mjs'];
...
export async function startMainBasis(root, project, task) {
  ...
  return { 状态: '已固定同步前版本', 下一步: '执行全局扫描与 context 同步，再 prepare-context、逐项复核、context；读取核对后的 context 后生成 MainBasis，prepare-basis、逐项复核、seal' };
}

async function transferInputs(root, config, task) {
  ...
  const excluded = scan.不读取内容 || [];
  check(Array.isArray(excluded) && excluded.every(row => upstream.some(f => f.路径 === row.路径)), '文件排除清单指向不存在的上游');
  check(excluded.every(row => !row.路径.startsWith(`${config.project}/context/`)), '全部 context 必须回读，不能整文件排除');
  const sources = await readUnits(root, upstream.filter(f => f.路径.startsWith(`${config.project}/`) && !derived.has(f.路径)), excluded);
  check(start.同步前派生文本?.length === derived.size, '缺少同步前派生文本，无法核对删除项');
  for (const before of start.同步前派生文本) {
    check(derived.has(before.路径) && hash(before.原文) === start.上游.find(f => f.路径 === before.路径)?.['SHA-256'], '同步前派生文本与原始哈希不符');
    sources.push(...textUnits(`同步前:${before.路径}`, before.原文));
  }
  const contextFiles = upstream.filter(f => f.路径.startsWith(`${config.project}/context/`));
  return { upstream, sources, contextFiles, targets: await readUnits(root, contextFiles) };
}

export async function prepareContext(root, project, task) {
  const config = await mainBasisConfig(root, project); check(config, '项目未配置 MainBasis');
  const inputs = await transferInputs(root, config, task);
  await write(path.join(taskPath(root, task), 'context-transfer.json'), seedTransfer(inputs.sources, inputs.targets), true);
  await write(path.join(taskPath(root, task), 'context-units.json'), { 来源: inputs.sources, 目标: inputs.targets }, true);
  return { 状态: '已建立全量 context 对照草稿，所有来源初始为未处理' };
}

export async function reviewContext(root, project, task) {
  const config = await mainBasisConfig(root, project); check(config, '项目未配置 MainBasis');
  const inputs = await transferInputs(root, config, task);
  const reportFile = `${task}/context-transfer.json`, report = await read(path.join(root, reportFile));
  validateTransfer(inputs.sources, inputs.targets, report);
  const checkpoint = { schemaVersion: '1.0', 项目目录: project, 上游: inputs.upstream, context: inputs.contextFiles,
    逐项核对: await fileRecord(root, reportFile), 时间: new Date().toISOString() };
  await write(path.join(taskPath(root, task), 'context-checkpoint.json'), checkpoint, true);
  return { 状态: 'context 已从磁盘完整回读并完成逐项对照；下一步从这些文件生成 MainBasis' };
}

async function checkedContext(root, project, task) {
  const checkpoint = await read(path.join(taskPath(root, task), 'context-checkpoint.json'));
  const config = await mainBasisConfig(root, project);
  check(checkpoint.项目目录 === project && digest(checkpoint.上游) === digest(await snapshot(root, config)), 'context 核对之后输入变化，须重新核对');
  check(digest(checkpoint.逐项核对) === digest(await fileRecord(root, `${task}/context-transfer.json`)), 'context 逐项核对记录已变化');
  return checkpoint;
}
...
export async function prepareBasis(root, project, task) {
  ...
  return { 状态: '已从核对后的全部 context 建立 MainBasis 对照草稿' };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  try {
    ...
    else throw new Error('用法：node scripts/mainbasis.mjs <verify|inspect|start|prepare-context|context|prepare-basis|seal|prepare-coverage> <项目相对目录> [新任务相对目录]');
