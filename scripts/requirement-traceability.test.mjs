...
test('覆盖入口核对场景契约、状态转换投影及真实候选', async () => {
  ...
  try {
    ...
    const referenced = structuredClone(report);
    referenced.契约模式 = '场景引用'; referenced.场景库SHA256 = fingerprint({场景:[scene]});
    for (const key of ['执行角色','目标端','入口','用例契约']) delete referenced.逐项[0].分支[0][key];
    await save(manifest.需求覆盖清单, reviewed(referenced));
    assert.equal((await validateCoverageInput(root, manifest, 'final')).交付性质, '完整覆盖');
    const beforeDedup = structuredClone(referenced);
    beforeDedup.逐项[0].分支.push({标识:'B2', 状态:'已设计', 说明:'另一个独立输入', 来源片段:source, 场景标识:'S2', 用例编号:'C1'});
    await save(manifest.需求覆盖清单, reviewed(beforeDedup));
    await assert.rejects(validateCoverageInput(root, manifest, 'final'), /不存在的独立场景/);
    await save(manifest.需求覆盖清单, reviewed(referenced));
    await save(manifest.独立场景库, {场景:[{...scene, 用例契约:{...contract,预期结果:['申请已通过']}}]});
    await assert.rejects(validateCoverageInput(root, manifest, 'final'), /当前完整场景库/);
    await save(manifest.独立场景库, {场景:[scene]});
    ...
    await save(manifest.独立场景库, { 场景: [scene] });
    const pendingTransition = structuredClone(report);
    pendingTransition.交付性质='部分覆盖';
    pendingTransition.状态转换处理=[{状态转换标识:'T1',状态:'待确认',说明:'观察范围缺少业务决定'}];
    await save(manifest.需求覆盖清单, reviewed(pendingTransition));
    await assert.rejects(validateCoverageInput(root,manifest,'final'),/状态转换待确认缺少问题编号/);
    pendingTransition.状态转换处理[0].问题编号='Q1';
    await save(manifest.需求覆盖清单, reviewed(pendingTransition));
    await assert.rejects(validateCoverageInput(root,manifest,'final'),/状态转换问题未进入最终/);
    await save(manifest.当前候选用例,{测试用例:[{用例编号:'C1',...contract}],需求待确认:[{问题编号:'Q1'}]});
    assert.equal((await validateCoverageInput(root,manifest,'final')).未完成项[0].状态转换标识,'T1');
