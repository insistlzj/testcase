...
const dependencies = [
  ...
  { 上游: "用户App/举报提交", 共同业务对象: "举报工单", 当前模块: "直播间举报", 下游: "管理后台/举报处理", 规则: "提交生成工单，后台处置通知用户的方式待确认", 证据: ["prototype/assets/annotations.js", "prototype/pages/admin/content/admin-report-detail.html"] },
  ...
  { 上游: "管理后台、公会App/运营账号", 共同业务对象: "虚拟金币送礼", 当前模块: "贡献榜、主播实时收益", 规则: "计入氛围和财富榜，不计入主播真实收益", 证据: ["context/02-公会App-项目需求清单.md", "context/03-管理后台-项目需求清单.md", "prototype/assets/annotations.js"] },
  { 上游: "用户App/主播结束直播", 共同业务对象: "直播场次", 当前模块: "直播记录", 下游: "公会App、管理后台直播记录", 规则: "同一场次形成用户端历史记录及其他端统计来源", 证据: ["context/01-业务对象清单.md", "context/01-用户主播App-项目需求清单.md", "context/02-公会App-项目需求清单.md", "context/03-管理后台-项目需求清单.md"] },
];
...
const result = {
  ...
  扫描模式: "full",
  模式依据: previousCache
    ? ["用户已明确本次使用组合方案；当前规则基线较缓存建立前发生变化，首次按新Schema重建全量缓存。"]
    : ["项目级缓存不存在，按组合方案首次执行full扫描。"],
  上次缓存: previousCache ? { 存在: true, 状态: previousCache.扫描状态 ?? "未知", 项目指纹: previousCache.项目指纹 ?? "" } : { 存在: false },
  规则基线: ruleBaseline,
  项目指纹: projectFingerprint,
  文件清单: manifest,
  文件变化: { 新增: manifest.map((item) => item.相对路径), 修改: [], 删除: [], 重命名候选: [] },
