...
test('导出只能使用登记的最终 JSON，不能覆盖既有文件或并发抢占的文件',async()=>{
  ...
  try {
    ...
    await assert.rejects(validateWorkbookExportPaths(task,root,path.join(task,'final.json'),path.join(root,'outputs/其他项目-case/new.xlsx')),/当前项目正式/);
    const stageOutput = path.join(task,'stage-exports/阶段性-用户App.xlsx');
    ...
    await assert.rejects(validateWorkbookExportPaths(task,root,path.join(task,'final.json'),stageOutput),/当前项目正式/);
