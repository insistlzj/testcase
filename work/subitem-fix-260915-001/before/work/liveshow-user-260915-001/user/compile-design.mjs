...
function subpoint(d,result){
 if(/协议仍/.test(result))return '协议勾选保留';
 if(/手机号输入仍/.test(result))return '号码输入保留';
 if(/密码保存成功/.test(result))return '密码保存';
 if(/不显示密码内容/.test(result))return '密码内容隐藏';
 if(/隐藏中间号码/.test(result))return '号码脱敏';
 if(/提示|文案/.test(result))return '反馈文案';
 if(/密码|验证码/.test(d.point))return /格式|长度|位|超长|非法/.test(d.point)?'凭证格式':'凭证有效性';
 if(/金币|余额|收益/.test(result))return '资产数值';
 if(/排名|排在|顺序|在前|名次/.test(result))return '排列次序';
 if(/进入|返回|跳转/.test(result))return '页面去向';
 if(/邀请|连麦/.test(d.point))return '连麦资格';
 if(/申请|审核|退会|入会/.test(d.point))return '申请处理';
 if(/好友|关注|粉丝团|拉黑/.test(d.point))return '账号关系';
 if(/按钮|不可|不能|无|隐藏/.test(result))return '入口可用性';
 if(/保存|保留|变为|清空|恢复/.test(result))return '数据保持';
 return '展示内容';
}
for(const [di,d] of drafts.entries()){
 ...
 for(const [ri,rawResult] of results.entries()){
  ...
  const rule={稳定规则标识:id,业务对象:page.name.replace(/^视图-/,'').replaceAll(' / ','-'),共同业务对象:page.name,执行角色:d.role,适用角色和端:[`用户App-${d.role}`],触发动作:d.steps.at(-1),必要条件:d.pre,来源状态:d.pre.slice(1).join('；')||`${d.role}位于${page.name}`,目标状态或可观察结果:result,规则状态:'已确认规则',可生成正式用例:true,原子动作数量:1,原子结果数量:1,功能模块:page.module,功能结构:`${page.name.replace(/^视图-/,'')}（${d.role}视角）`,模块归属说明:`当前MainBasis目录：${page.path}属于${page.module}`,证据引用:[...refs.map(r=>({...r.source,证明内容:r.text})),{...page.entry,证明内容:'当前目标端实际页面及承载位置'}],
   ...
   用例设计:{场景:`验证${d.point}${reviewed?`：${reviewed[ri][0]}`:results.length>1?`（${ri+1}）`:''}`,验证子项:reviewed?.[ri]?.[0]||subpoint(d,result),观察页面:page.name,观察页面路径:page.path,观察对象:d.point,观察端:'用户App',设计说明:`从当前MainBasis条款${d.refs.join('、')}设计；本条观察${result}`,观察证据:refs.map((_,i)=>i),步骤:steps,计算:calculation,用例类型:d.type,优先级:d.priority},
  };
