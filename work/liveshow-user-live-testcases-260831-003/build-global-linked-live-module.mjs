...
const outputPath = path.join(outputDir, "用户App-直播模块-260831-003.xlsx");
const jsonPath = path.join(workDir, "用户App-直播模块-测试用例-260831-003.json");
...
const inspectionPath = path.join(workDir, "inspection-260831-003.json");
...
assert.equal(scanResult.扫描模式, "fallback-full");
...
const syncResult = {
  ...
  同步摘要: [
    ...
    "本次因 6 个旧依赖文件被删除且替代关系未声明，按 fallback-full 对 259 个项目文件重新建档。",
...
const syncNote = "同步追溯：work/liveshow-user-live-testcases-260831-003/prototype-context-sync-result.json；RSL-0002、RSL-0003";
const globalEvidenceNote = "全局证据追溯：work/liveshow-user-live-testcases-260831-003/global-evidence-scan-result.json；扫描模式 fallback-full";
