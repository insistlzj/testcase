...
const workDir = path.join(rootDir, "work/liveshow-user-live-testcases-260901-002");
const outputDir = path.join(rootDir, "outputs/Luma Live-case");
const outputPath = path.join(outputDir, "用户App-直播模块-260901-002.xlsx");
const jsonPath = path.join(workDir, "用户App-直播模块-测试用例-260901-002.json");
...
const inspectionPath = path.join(workDir, "inspection-260901-002.json");
...
const newTraceDir = "work/liveshow-user-live-testcases-260901-002";
const executionFields = ["前置条件", "操作步骤", "预期结果"];
const businessAliases = [
  { code: "GFT-LIVE-01", name: "“鲜花”", source: "用户App直播间原型礼物列表" },
  { code: "GFT-LIVE-02", name: "“咖啡”", source: "管理后台 Mock 的下架普通礼物" },
  { code: "GFT-LIVE-03", name: "“星光”", source: "用户App直播间原型礼物列表" },
  { code: "GFT-LIVE-04", name: "“麦克风”", source: "用户App直播间原型礼物列表" },
  { code: "HOST-G01", name: "“公会权限测试主播”", source: "本次测试数据准备约定" },
  { code: "HOST-NORMAL-01", name: "“普通主播”", source: "本次测试数据准备约定" },
  { code: "HOST-TEST-01", name: "“灰度测试主播”", source: "本次测试数据准备约定" },
  { code: "LIVE-ADMIN-001", name: "“平台关播测试场次”", source: "本次测试数据准备约定" },
  { code: "LIVE-ADMIN-T01", name: "“平台关播门票房场次”", source: "本次测试数据准备约定" },
  { code: "LIVE-OPS-001", name: "“运营送礼测试场次”", source: "本次测试数据准备约定" },
  { code: "LIVE-REPORT-001", name: "“举报测试场次”", source: "本次测试数据准备约定" },
  { code: "LIVE-SESSION-001", name: "“直播记录测试场次”", source: "本次测试数据准备约定" },
  { code: "LIVE-SESSION-002", name: "“深夜聊天局”", source: "原用例已明确的开播标题" },
  { code: "OP-LIVE-01", name: "“气氛号A”", source: "本次测试数据准备约定" },
  { code: "OP-LIVE-02", name: "“免检账号B”", source: "本次测试数据准备约定" },
];
...
function applyReadableBusinessAliases(item) {
  const seen = new Set();
  for (const field of executionFields) {
    item[field] = item[field].map((value) => {
      ...
      for (const alias of businessAliases) {
        ...
        const parts = result.split(alias.code);
        result = parts[0];
        for (const part of parts.slice(1)) {
          result += seen.has(alias.code) ? alias.name : `${alias.name}（数据别名：${alias.code}）`;
          seen.add(alias.code);
          result += part;
        }
        result = result.replaceAll(` ${alias.name}`, alias.name);
      }
      return result;
    });
  }
}
for (const item of cases) applyReadableBusinessAliases(item);
...
for (const [index, item] of cases.entries()) {
  ...
  for (const code of new Set(executionCodes)) {
    ...
    assert(executionText.includes(`${alias.name}（数据别名：${code}）`), `业务对象编码没有随名称解释：${item.用例编号} / ${code}`);
  }
}
...
const aliasReview = {
  ...
  功能模块: "直播模块",
  判定时间: formatShanghaiTime(new Date()),
  表达规则: "业务对象名称优先；同一用例首次出现时附数据别名，后续只写名称；结构化追踪编号保持不变。",
...
syncResult.同步摘要 = [
  "按 RCL-0023 将正式执行字段中的业务对象改为名称优先表达，数据别名仅在本条用例首次出现时随名称解释；结构化追踪编号保持不变。",
...
syncResult.受影响用例.push(
  ...
  { 范围: `用户App-直播模块-260901-001.xlsx / ${compressIds(readableAliasAffectedIds).join("、")}`, 状态: "继续有效", 数量: readableAliasAffectedIds.length, 说明: "业务规则未变；在新版本中改为名称优先、数据别名首次解释" },
);
...
const scanResult = {
  ...
  扫描模式: "full",
  模式依据: ["AGENTS.md 和 Cem Kaner.txt 新增 RCL-0023 业务对象名称优先与数据别名解释规则，规则基线变化触发全量扫描。", "继续执行 RCL-0022 目标端可执行性与规则归属门禁，并重新核对幸运礼物的系统概要、派生需求、用户App共享交互、直播间动态状态和批注。"],
...
const inspection = {
  ...
  readableAliases: (await finalWorkbook.inspect({ kind: "match", searchTerm: "数据别名：", options: { useRegex: false, maxResults: 100 }, summary: "业务对象名称优先与数据别名解释" })).ndjson,
...
for (const [sheetName, range, fileName] of [
  ...
  ["功能测试用例", "A348:O358", "preview-readable-aliases-gifts.png"],
