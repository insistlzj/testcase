...
const taskDir = path.resolve(root, process.argv[2] || "work/liveshow-user-live-discovery-260910-005");
const sourcePath = path.join(taskDir, process.argv[3] || "用户App-直播模块-测试用例-260910-003.json");
const outputPath = path.resolve(root, process.argv[4] || "outputs/Luma Live-case/用户App-直播模块-260910-003.xlsx");
...
overview.getRange("A1").values = [["用户App-直播模块 产品决策概览"]];
...
for (const [name, sheetName, range] of [
  ...
  ["pending", "需求待确认", `A1:T${pendingLastRow}`],
]) {
