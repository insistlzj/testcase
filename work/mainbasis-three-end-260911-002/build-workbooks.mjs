...
const taskRoot = path.join(root, 'work/mainbasis-three-end-260911-002');
const outputDir = path.join(root, 'outputs/Luma Live-case');
const ends = {
  用户App: { dir: 'user', json: 'final.json', table: 'UserAll' },
  公会App: { dir: 'guild', json: 'final.json', table: 'GuildAll' },
  管理后台: { dir: 'admin', json: 'final.json', table: 'AdminAll' },
};
...
function styleSheet(sheet, headers, values, columnWidths, tableName) {
  ...
  for (const [i, row] of values.entries()) {
    const maxLines = Math.max(...row.map((value,j) => String(value).split('\n').reduce((sum,line)=>sum+Math.max(1,Math.ceil([...line].reduce((n,ch)=>n+(ch.charCodeAt(0)>255?2:1),0)/Math.max(4,columnWidths[j]-2))),0)));
    sheet.getRange(`A${i+2}:${lastColumn}${i+2}`).format.rowHeight = Math.min(350,Math.max(54,maxLines*14+12));
  }
...
async function nextOutput(endName) {
  const planPath=path.join(taskRoot,'excel-plan.json');
  const plan=JSON.parse(await fs.readFile(planPath,'utf8').catch(()=>'{}'));
  if(plan[endName])return plan[endName];
  ...
  const chosen=path.join(outputDir, `${endName}-全部模块-260911-${String(Math.max(0, ...used) + 1).padStart(3, '0')}.xlsx`);
  plan[endName]=chosen;
...
async function build(endName) {
  ...
  assert.deepEqual(JSON.parse(await fs.readFile(path.join(taskDir,'candidate.json'),'utf8')),source,'当前数据与导出快照不一致');
  ...
  overview.getRange('A2').values = [['MainBasis 现有数据直接导出；未追加内容复核；用例均未测']];
  ...
  overview.getRange('A12:D12').values = [['用例总数','通过','不通过','未测']];
  overview.getRange('A13:D13').formulas = [[`=COUNTA('功能测试用例'!$B$2:$B${caseLast})`,`=COUNTIF('功能测试用例'!$M$2:$M${caseLast},"通过")`,`=COUNTIF('功能测试用例'!$M$2:$M${caseLast},"不通过")`,`=COUNTIF('功能测试用例'!$M$2:$M${caseLast},"未测")`]];
  overview.getRange('A12:D12').format = header;
  overview.getRange('A13:D13').format = {font:{name:'Arial',size:11},horizontalAlignment:'center',borders:border};
  ...
  await fs.writeFile(path.join(taskDir, 'delivery-verification.json'), `${JSON.stringify({ schemaVersion: '1.0', 状态: differences ? '不通过' : '通过', JSON一致性差异数: differences,
    工作簿SHA256: await hashFile(outputPath), 候选SHA256: await hashFile(path.join(taskDir, 'candidate.json')),
  ...
  for (const [name, sheetName, range] of [['overview', '产品决策概览', 'A1:H13'], ['cases-top', '功能测试用例', 'A1:O7'], ['cases-bottom', '功能测试用例', `A${Math.max(1, caseLast - 4)}:O${caseLast}`], ['pending', '需求待确认', 'A1:T7']]) {
...
await fs.writeFile(path.join(taskRoot,'excel-outputs.json'),JSON.stringify(results,null,2)+'\n');
