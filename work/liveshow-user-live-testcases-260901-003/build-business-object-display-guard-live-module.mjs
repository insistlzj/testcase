...
const workDir = path.join(rootDir, "work/liveshow-user-live-testcases-260901-003");
const outputDir = path.join(rootDir, "outputs/Luma Live-case");
const outputPath = path.join(outputDir, "用户App-直播模块-260901-003.xlsx");
const jsonPath = path.join(workDir, "用户App-直播模块-测试用例-260901-003.json");
...
const inspectionPath = path.join(workDir, "inspection-260901-003.json");
...
const newTraceDir = "work/liveshow-user-live-testcases-260901-003";
...
syncResult.同步摘要 = [
  "按 RCL-0024 修复业务对象显示：有来源名称继续使用名称；无来源名称改用对象描述和数据别名，后续使用该对象类指代；备注执行同一解释规则。",
  "已区分直播场次与开播标题等字段值，并清理中文右引号、右全角括号后的多余空格；未修改业务规则、用例数量和需求清单。",
...
syncResult.受影响用例.push(
  ...
  { 范围: `用户App-直播模块-260901-002.xlsx / ${compressIds(readableAliasAffectedIds).join("、")}`, 状态: "继续有效", 数量: readableAliasAffectedIds.length, 说明: "业务规则未变；新版本按 RCL-0024 修复无来源名称、备注裸编码、字段值误作名称和显示空格" },
);
...
const scanResult = {
  ...
  扫描模式: "full",
  模式依据: ["AGENTS.md 和 Cem Kaner.txt 新增 RCL-0024 无来源业务对象命名与显示质量门禁，规则基线变化触发全量扫描。", "继续执行 RCL-0022 目标端可执行性与规则归属门禁，并重新核对幸运礼物的系统概要、派生需求、用户App共享交互、直播间动态状态和批注。"],
