...
const taskRoot = path.join(root, 'work/mainbasis-three-end-260911-001');
...
async function build(endName) {
  ...
  await validateGenerationInput(path.join(taskDir,'generation-input-manifest.json'),root,'final');
  ...
  overview.getRange('A2').values = [['MainBasis 需求基准；用例未执行；问题数量由明细实时汇总']];
  ...
  for (const [name, sheetName, range] of [['overview', '产品决策概览', 'A1:H10'], ['cases-top', '功能测试用例', 'A1:O7'], ['cases-bottom', '功能测试用例', `A${Math.max(1, caseLast - 4)}:O${caseLast}`], ['pending', '需求待确认', 'A1:T7']]) {
