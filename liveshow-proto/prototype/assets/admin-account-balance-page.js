(function () {
  ...
  const configs = {
    host: {
      ...
      cells: (item) => {
        ...
        return [`<b>${escapeHtml(item.hostName)}</b><span class="admin-cell-sub">${escapeHtml(item.hostId)}</span>`, `<b>${escapeHtml(item.guildName)}</b><span class="admin-cell-sub">${escapeHtml(item.guildId)}</span>`, `<a class="admin-action admin-balance-link" href="${href}">${format.money(item.balance)}</a>`, `<a class="admin-action" href="${href}">变更记录</a>`];
      }
    },
    guild: {
      ...
      cells: (item) => {
        ...
        return [`<b>${escapeHtml(item.guildName)}</b><span class="admin-cell-sub">${escapeHtml(item.guildId)}</span>`, `<a class="admin-action admin-balance-link" href="${href}">${format.money(item.balance)}</a>`, `<a class="admin-action" href="${href}">变更记录</a>`];
      }
    }
  };
  ...
  function balanceList(config) {
    ...
    root.innerHTML = `
      ...
      <section class="raw-report-kpis account-balance-summary" aria-label="${config.title}汇总"><div class="raw-report-kpi"><span>账号余额汇总</span><b id="balanceTotal"></b></div></section>
  ...
  function balanceChangeRecords(kind) {
    ...
    const config = isHost ? {
      ...
      idParam: 'hostId', idKey: 'hostId', nameKey: 'hostName', rows: mock.hostAccountBalances, records: mock.hostBalanceChanges, incomeType: '收益分成', recordPrefix: 'HBC'
    } : {
      ...
      idParam: 'guildId', idKey: 'guildId', nameKey: 'guildName', rows: mock.guildAccountBalances, records: mock.guildBalanceChanges, incomeType: '收益分成', recordPrefix: 'GBC'
    };
    ...
    root.innerHTML = `
      ...
      <section class="admin-panel admin-profile-panel admin-balance-overview">
        <div class="admin-profile-summary"><div class="admin-balance-account"><h2>${escapeHtml(accountName)}</h2><div class="admin-profile-meta">${profileMeta}</div></div><div class="admin-metrics"><div class="admin-metric"><span>余额</span><b id="currentBalance">${format.money(account.balance)}</b></div></div></div>
      </section>
      <section class="admin-panel admin-filter" aria-label="余额变更记录筛选"><div class="admin-filter-grid"><div class="admin-field"><label for="filterType">变更类型</label><select class="admin-input" id="filterType"><option value="">全部类型</option><option>${config.incomeType}</option><option>分成修正</option></select></div><div class="admin-field"><label>变更时间</label><div class="admin-range admin-date-range"><input class="admin-input" id="filterStart" type="date"><span>至</span><input class="admin-input" id="filterEnd" type="date"></div></div></div><div class="admin-filter-actions"><button class="admin-btn secondary" id="resetBtn" type="button">重置</button><button class="admin-btn" id="searchBtn" type="button">查询</button></div></section>
    ...
    function render(items) {
      pagination.classList.toggle('state-hide', !items.length);
      rows.innerHTML = items.length ? items.map((item) => { const direction = item.type === config.incomeType ? 'in' : item.direction; return `<tr><td>${escapeHtml(item.recordNo)}</td><td>${escapeHtml(item.createdAt)}</td><td><span class="admin-status">${escapeHtml(item.type)}</span></td><td class="balance-change-amount ${direction === 'in' ? 'is-positive' : 'is-negative'}"><b>${direction === 'in' ? '+' : '-'}${format.money(item.amount)}</b></td><td>${format.money(item.balanceBefore)}</td><td><b>${format.money(item.balanceAfter)}</b></td><td>${escapeHtml(item.operator)}</td><td>${escapeHtml(item.remark)}</td></tr>`; }).join('') : '<tr><td colspan="8"><div class="admin-empty"><strong>暂无余额变更记录</strong><span>可新建一条分成修正记录</span></div></td></tr>';
    }
