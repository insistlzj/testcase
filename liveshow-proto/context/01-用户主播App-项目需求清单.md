...
## 公共业务规则
- [COMMON-user-date-format] 原型说明约定用户 App 日期时间采用 dd/mm/yyyy HH.mm，纯日期不附加时间；注销冷静期批注另写 MM/DD HH:mm，该页面格式冲突见 PENDING-cooling-date-format。格式不决定业务时区，业务日边界仍按 PENDING-business-zone 待确认。〔依据：prototype/Luma Live-原型说明.md / 视觉规则；prototype/annotations/user.js / views/auth-login-register/deletion-cooling.html〕
...
- [REQ-d8abb53c2c28] 贡献榜的真实金币贡献按送礼用户汇总；运营虚拟金币是否计入本榜按 PENDING-virtual-board 待确认，不采用与系统概要相反的全量排除结论。〔来源：AN-0e624b2dc753；liveshow-proto/prototype/annotations/user.js；contribution-ranking.html/业务规则/2〕
...
- [REQ-e6f7cdd9a68b] 开始直播：未具备主播身份时进入申请成为主播 P014-1；已具备主播身份时直接进入开播设置还是先进入主播中心，原说明两种说法并存，见 PENDING-profile-live-entry。〔来源：AN-52423cc339e3；liveshow-proto/prototype/annotations/user.js；profile.html/交互/10〕
...
- [REQ-c26340580604] 当前直播场次已关闭，观众不能继续观看、评论或送礼；主播连麦结束由主播视角规则承接，不表示观众有发起连麦入口。〔来源：AN-584b9d39f8fb；liveshow-proto/prototype/annotations/user.js；live-end-viewer.html/业务/1；context/系统概要 .md/3.2.1、3.3.4〕
...
- [REQ-ac7a51ae49a5] 原型演示说明（非业务规则）：同一会话点击注销交替展示两种余额情形，仅用于演示；实际注销判断采用当前账号真实金币余额，不采用交替次数。〔来源：AN-5450609415f9；liveshow-proto/prototype/annotations/user.js；settings.html/交互/5〕
...
- [REQ-0dc836b4c15d] 统计范围：当前直播场次的真实金币有效贡献包含当前在线和已经离线的用户；运营虚拟金币榜单范围见 PENDING-virtual-board。〔来源：AN-fa23a2a2adc8；liveshow-proto/prototype/annotations/user.js；views/live-room-host/contribution-rank.html/字段/3〕
...
- [REQ-97187d7ee032] 本场贡献榜按当前场次累计真实金币贡献；用户离开直播间后仍保留本场贡献值和排名。运营虚拟金币范围见 PENDING-virtual-board。〔来源：AN-e062dbd4ae13；liveshow-proto/prototype/annotations/user.js；views/live-room-host/contribution-rank.html/业务/1〕
...
- [REQ-fed69bde9ff8] 任一方结束直播或变更为受限房型时，不再满足条件的邀请失效。接受邀请并建立连麦后，自己发出的邀请失效，其他收到的邀请保留；连麦中不能接受其他邀请或发起新邀请。〔来源：AN-78012834aa35；liveshow-proto/prototype/annotations/user.js；views/live-room-host/cohost-hosts.html/业务/4；context/系统概要 .md/3.3.4，以系统概要覆盖进入连麦后全部邀请失效的歧义〕
...
- [REQ-6fa3d081c1f0] 仅统计已形成有效记录的直播场次；场次结束后数据保留。充值退款或拒付不撤销已完成的礼物和门票消费，对应主播收益和分成不受影响；失败或撤销的消费不计入主播收益。〔来源：AN-bb55052160e7；liveshow-proto/prototype/annotations/user.js；views/live-data/month.html/业务规则/1；context/系统概要 .md/3.6.1〕
